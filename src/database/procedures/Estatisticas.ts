export function Estatisticas() {
  return `
    ...
    BEGIN
        ...
        SELECT p.name AS Question1 FROM product p INNER JOIN (
        ...
        SELECT e.name AS Question2 FROM employee e INNER JOIN (
        ...
        SELECT p.name AS Question3 FROM product p INNER JOIN (
        ...
        SELECT SUM(s.amount * p.value) AS Question4 FROM sale s INNER JOIN product p ON s.id_product = p.id INNER JOIN (
            SELECT id_product, SUM(amount) AS total_amount FROM sale GROUP BY id_product ORDER BY total_amount DESC
            LIMIT 1
        ...
        SELECT MONTH(date) AS Question5, COUNT(*) AS TotalSalesLargestMonth FROM sale WHERE id_product = (
            SELECT id_product FROM (
                SELECT id_product, SUM(amount) AS total_amount FROM sale GROUP BY id_product ORDER BY total_amount DESC
                LIMIT 1
            ) t
        )
        GROUP BY MONTH(date) ORDER BY TotalSalesLargestMonth DESC
        LIMIT 1;
    
        -- 6 Mês de menor vendas do produto mais vendido
    
        SELECT MONTH(date) AS Question6, COUNT(*) AS TotalSalesSmallestMonth FROM sale WHERE id_product = (
            SELECT id_product FROM (
                SELECT id_product, SUM(amount) AS total_amount FROM sale GROUP BY id_product ORDER BY total_amount DESC
                LIMIT 1
            ) t
        )
        GROUP BY MONTH(date) ORDER BY TotalSalesSmallestMonth ASC
        LIMIT 1;
    
        -- 7 Valor ganho com o produto menos vendido
    
        SELECT SUM(s.amount * p.value) AS Question7 FROM sale s INNER JOIN product p ON s.id_product = p.id INNER JOIN (
            SELECT id_product, SUM(amount) AS total_amount FROM sale GROUP BY id_product ORDER BY total_amount ASC
            LIMIT 1
        ) t ON s.id_product = t.id_product;
        ...
        SELECT MONTH(date) AS Question8, COUNT(*) AS TotalSalesLargestMonth FROM sale WHERE id_product = (
            SELECT id_product FROM (
                SELECT id_product, SUM(amount) AS total_amount FROM sale GROUP BY id_product ORDER BY total_amount ASC
                LIMIT 1
            ) t
        ...
        SELECT MONTH(date) AS Question9, COUNT(*) AS TotalSalesSmallestMonth FROM sale WHERE id_product = (
            SELECT id_product FROM (
                SELECT id_product, SUM(amount) AS total_amount FROM sale GROUP BY id_product ORDER BY total_amount ASC
                LIMIT 1
            ) t
